"""
Kazi - Voice-first AI Agent via WhatsApp with Reminders
"""
...
KAZI_SYSTEM = """You are Kazi, a helpful AI assistant via WhatsApp. Keep responses short (under 300 chars).

YOU CAN SET REMINDERS! When a user asks for a reminder, confirm it AND add this JSON block at the very end:

REMINDER_JSON:{"task":"call Emma","hour":15,"minute":0}

Rules:
- Extract the task description
- Convert time to 24-hour format (3pm = 15, 9am = 9)
- Always include hour and minute
- Current time: {current_time}

Example: "remind me to call Emma at 3pm" -> Confirm, then add REMINDER_JSON:{"task":"call Emma","hour":15,"minute":0}"""
...
async def save_reminder(user_phone: str, task: str, hour: int, minute: int):
    if db_pool:
        now = datetime.now()
...
async def get_response(user_message: str, user_phone: str) -> str:
