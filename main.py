...
@app.post("/webhook")
async def whatsapp_webhook(From: str = Form(...), Body: str = Form(default=""), NumMedia: str = Form(default="0"), MediaUrl0: str = Form(default=None), MediaContentType0: str = Form(default=None)):
    user_phone = From
    print(f"Message from {user_phone}: {Body}")
    try:
        ...
        kazi_response = await get_kazi_response(user_message)
        await send_whatsapp_message(user_phone, kazi_response)
        return Response(content="<Response></Response>", media_type="text/xml")
    except Exception as e:
        print(f"Error: {e}")
        await send_whatsapp_message(user_phone, "Sorry, something went wrong. Please try again.")
        return Response(content="<Response></Response>", media_type="text/xml")
