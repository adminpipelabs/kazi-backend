...
KAZI_SYSTEM = "You are Kazi, a helpful AI assistant via WhatsApp. Keep responses short. YOU CAN SET REMINDERS! When user asks for a reminder, confirm it AND add this at the end: REMINDER_JSON:{\"task\":\"the task\",\"hour\":15,\"minute\":0} - Convert time to 24h format. Current time: {current_time}"
...
async def check_reminders():
    ...
    while True:
        try:
            if db_pool:
                async with db_pool.acquire() as conn:
                    rows = await conn.fetch("SELECT id, user_phone, task FROM reminders WHERE remind_at <= NOW() AND sent = FALSE")
                    for r in rows:
...
async def save_reminder(user_phone, task, hour, minute):
    if db_pool:
        ...
        async with db_pool.acquire() as conn:
            await conn.execute("INSERT INTO reminders (user_phone, task, remind_at) VALUES ($1, $2, $3)", user_phone, task, remind_at)
        print(f"Saved: {task} at {remind_at}")

async def get_response(user_message, user_phone):
    current_time = datetime.now().strftime("%Y-%m-%d %H:%M")
    response = claude.messages.create(model="claude-sonnet-4-20250514", max_tokens=500, system=KAZI_SYSTEM.replace("{current_time}", current_time), messages=[{"role": "user", "content": user_message}])
    ...
    if "REMINDER_JSON:" in text:
        try:
            ...
            await save_reminder(user_phone, data["task"], data["hour"], data["minute"])
            text = text[:idx].strip()
        except Exception as e:
