...
📱 You have 10 free messages per day — ask anything, set reminders, chat. Resets tomorrow!"""
...
REMINDER_JSON:{"task":"description","hour":15,"minute":0}

TIME INTERPRETATION (use user's local time):
- "this morning" = 8:00 AM (08:00)
- "this afternoon" = 2:00 PM (14:00)
- "this evening" / "tonight" = 7:00 PM (19:00)
- "later" = 2 hours from current time
- "in a bit" = 30 minutes from current time
- "in X hours/minutes" = current time + X
- Always convert to 24h format for the JSON
...
TIME QUERIES: If user asks "what time is it" or similar, tell them the current time based on Current time shown above."""
...
async def save_reminder(user_phone, task, hour, minute, tz_name):
    if db_pool:
        ...
        print(f"Saved: {task} at {remind_utc} UTC (local was {remind_local})")
        return True
    return False
...
async def get_response(user_message, user_phone):
    ...
    system = KAZI_SYSTEM.replace("{current_time}", current_time).replace("{timezone}", user_tz or "UTC")
